...
<html>
<head>
	<title>Manage Students - YREMS</title>
...
<body style="background: white;">
    ...
    <div id="content_div" class="content_div" style="display: none;">
        <div class="col-7">
            ...
            <p style="line-height: 1.4; color: gray; font-size: 90%;">
                <b>What to do with new users</b><br />
                1. If they ARE Zellers, but just not in your RA group, leave them in "waiting approval”(i.e. do nothing) so that other RAs can edit the information and accept those students themselves.<br />
                2. If they are in your RA group, just with the wrong information, accept them for now and post a notice in the RA group saying that they should change their wrong information soon, or they might have problems getting points/signing up.<br />
                3. If they are just weird random users, definitely decline them.<br /><br />
                4. “Decline” deletes their entire account data, so they will have to enter everything again. So use this option for people who are not Zellers at all. <br />
                5. “Block user” blocks their login without deleting any data, and therefore should be used on people on “the list.” They will be redirected to a <span onclick="window.open('blocked.php');" style="color: blue">page</span> explaining the cancellation policy, and setting up office hours with the RM.<br />
            </p>
            ...
            <?php
                if($_SESSION["user_type"]=="Chief RA" || $_SESSION["user_type"]=="RM") {
                    echo '<button class="blue small" onclick="manage_ra()">Manage RAs</button>';
                    echo '<script>function manage_ra() { window.location="/yicrc/manage_ra_2.php"; } </script>';
                }
            ?>
            
            <p id="my_students_title" style="color: #0E69B1">My Students</p>
            <div id="my_students_div"></div>
        ...
        <div class="col-5">
            ...
            <button id="activity_btn" onclick="get_activity()" class="blue_border" style="display: none;">Load RC Activities</button>
            <h2><br />Student RC Activities</h2>
            <p id="sel_user_p"></p>
    ...
    <script>
        ...
        function clear_search() {
	        ...
	        if(gei("my_table")!=null) {
	            var my_table = gei("my_table").children[0].children;   
	        }
	    ...
	    function search_part() {
	        ...
	        if(query=="") {
	            ...
	            if(gei("my_table")!=null) {
	                for(i=0;i<my_table.length;i++) {
	                    show=false;
	                    for(j=0; j<my_table[i].children.length;j++) {
	                        if(my_table[i].children[j].innerHTML.toUpperCase().includes(query.toUpperCase())) {
	                            show=true;
	                        }
	                    }
	                    if(show) {
        ...
        function get_stu(showall) {
            ...
            if(showall) {
                $.ajax({
                    url: "eng/manage_students_load.php",
                    success: function(data) {
                        ...
                        if(stu!=null) {
                            var out_text_waiting = '<table id="waiting_table" class="user" style="width: 100%;"><tr><td>#</td><td>Name</td><td>ID</td><td>Major</td><td>RA</td><td>Status</td><td>Action</td></tr>'; //#, Name, ID, Major, RA, Status, Action
                            ...
                            var out_text_all = '<table id="all_table" class="user" style="width: 100%;"><tr><td>#</td><td>Name</td><td>ID</td><td>Major</td><td>RA</td><td>Room</td><td>Status</td></tr>'; // #, Name, ID, Major, RA, Room, Status
                            ...
                            if(Object.keys(stu).length==0) {
                                out_text_my += '<tr><td colspan="7" style="color: red">No students.</td></tr>';
                                ...
                                for(i=0;i<Object.keys(stu).length;i++) {
                                    if(stu[i].user_accepted==1) {
                                        ...
                                        out_text_all += '<td>'+ra_name(stu[i].user_ra)+'</td>';
                                        out_text_all += '<td>'+stu[i].user_room+'</td>';
                                        out_text_all += '<td>'+wait_reason_text[stu[i].user_accepted]+'</td>';
                                        out_text_all += '</tr>';
                                        if(stu[i].user_ra==my_id) {
                                            my_num++;
                                            out_text_my += '<tr style="cursor: pointer;" onclick="stu_click('+stu[i].user_id+')">';
                                            out_text_my += '<td>'+my_num+'</td>';
                                            out_text_my += '<td>'+stu[i].user_eng_name_first+' '+stu[i].user_eng_name_last+' ('+stu[i].user_name+')</td>';
                                            out_text_my += '<td>'+stu[i].user_id+'</td>';
                                            out_text_my += '<td>'+stu[i].user_major+'</td>';
                                            //out_text_my += '<td>'+ra_name(stu[i].user_ra)+'</td>';
                                            out_text_my += '<td>'+wait_reason_text[stu[i].user_accepted]+'</td>';
                                            out_text_my += '<td>'+stu[i].user_room+'</td>';
                                            out_text_my += '<td>'+stu[i].user_mobile+'</td>';
                                            out_text_my += '<td><button class="red" style="width: auto;padding: 5px;" onclick="delete_user('+stu[i].user_id+')">Delete</button>';
                                            out_text_my += '</tr>';
                                        }
                                    } else {
                                        wait_num++;
                                        ...
                                        out_text_waiting += '<td>'+ra_name(stu[i].user_ra)+'</td>';
                                        ...
                                        if(stu[i].user_ra==my_id) {
                                            ...
                                            out_text_my += '<td>';
                                            if(stu[i].user_accepted==2 || stu[i].user_accepted==3) { //new
                                                out_text_my += '<button class="green" style="width: auto;padding: 5px;" onclick="accept_user('+stu[i].user_id+',1)">Accept</button><button class="red" style="width: auto;padding: 5px;" onclick="delete_user('+stu[i].user_id+')">Decline</button>'
                                            } else {
                                                if(gei("user_type_div").innerText=="RM") {
                                                    out_text_my += '<button class="green" style="width: auto;padding: 5px;" onclick="accept_user('+stu[i].user_id+',1)">Accept</button><button class="red" style="width: auto;padding: 5px;" onclick="delete_user('+stu[i].user_id+')">Decline</button>'
                                                } else {
                                                    out_text_my += stu[i].user_room;
                                                }
                                            }
                                            out_text_my += '</td>';
                                            out_text_my += '<td>'+stu[i].user_mobile+'</td>';
                            ...
                            gei("my_students_title").innerHTML = "My Students ("+my_num+")";
                            if(my_num==0) {
                                gei("my_students_div").style.display="none";
                            } else {
                                gei("my_students_div").innerHTML = out_text_my;
                                gei("my_students_div").style.display="";
                            }

                            gei("house_students_title").innerHTML = "<br />All House Students ("+all_num+")";
                            if(all_num==0) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if(stu!=null) {
                                    ...
                                    if(stu[i].user_accepted==1) {
                                        ...
                                        if(stu[i].user_ra==my_id) {
                                            ...
                                            if(gei("user_type_div").innerText=="RM" || gei("user_type_div").innerText=="Chief RA") {
                                                ...
                                                //out_text_all += '<tr>';   
                                                out_text_all += '<tr onclick="stu_click('+stu[i].user_id+')">'; //for registration period only
                                            }
                                        }
                                        out_text_all += '<tr onclick="stu_click('+stu[i].user_id+')">';
                                        out_text_all += '<td>'+all_num+'</td>';
                            ...
                            /*
                            gei("house_students_title").innerHTML = "All House Students ("+all_num+")";
                            if(all_num==0) {
                                gei("house_students_title").style.display="none";
                                gei("house_students_div").style.display="none";
                            } else {
